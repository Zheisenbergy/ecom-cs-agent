...
7. benchmark 本身也需要持续校准，不能只看第一次输出

## 7. 现在最推荐的下一步

当前最推荐的顺序仍然是：

1. 先把 `mixed v4` 作为当前最佳 router 版本固定下来
2. 增加一套更难的 holdout benchmark，确认不是只在当前 mixed dev 上变强
3. 如果 holdout 也稳，再决定是继续训 `answer` 还是做联调

换句话说：

- 当前已经不需要继续盲目堆 router 数据
- 更值得优先做的是：
  - 泛化验证
  - holdout benchmark
  - answer 阶段训练或主链路联调
