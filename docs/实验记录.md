...
这份文档记录当前项目到 `2026-04-05` 为止，已经实际做过的训练、benchmark、遇到的问题，以及对应的处理方式。
...
### 3.6 mixed v4：prompt 对齐 + ask_user 上下文对照样本后的训练

这轮改动的核心不是继续盲目扩数据，而是：

1. 让 router 训练 prompt 和 benchmark prompt 复用同一套规则
2. 加入“同一句模糊问法，有上下文时不追问、没上下文时必须追问”的对照样本
...
| 指标 | mixed v4 |
|---|---:|
| `route_accuracy` | 0.9358 |
| `route_macro_f1` | 0.8950 |
| `intent_accuracy` | 0.9134 |
| `intent_macro_f1` | 0.9082 |
| `tool_accuracy` | 0.9190 |
| `tool_macro_f1` | 0.8764 |
| `ask_user_f1` | 0.8870 |
| `handoff_f1` | 0.9818 |
| `missing_slots_exact_match` | 0.9246 |
| `tool_arguments_exact_match` | 0.9246 |

这轮结论：

- `ask_user` 从“当前最大短板”变成了“已经基本学会”
- `handoff` 进一步接近满分
- `route / intent / tool` 也一起大幅提升
- `missing_slots` 和 `tool_arguments` 同步变稳，说明模型不是只会“问”，而是问得更准、填得也更准

需要保持清醒的一点：

- 这轮 dev 集总量变成了 358，不是和 `mixed v3` 完全相同的一张考卷
- 所以这轮结果虽然方向上毫无疑问是大幅提升，但后面仍值得补一套更难的 holdout benchmark 做泛化验证

### 3.7 answer mixed v1：第一轮 mixed answer 训练

在 `router mixed v4` 稳住之后，继续训练了第一版 `answer` LoRA。
...
| 指标 | answer mixed v1 |
|---|---:|
| `answer_token_f1` | 0.9496 |
| `answer_exact_match` | 0.8911 |
| `citation_set_accuracy` | 1.0000 |
| `grounded_f1` | 1.0000 |
| `escalation_f1` | 1.0000 |

这轮结论：

- `answer` 已经明显学会基于 observation 做 grounded 回答
- 工具引用集完全正确，没有乱引用
- 需要升级人工的场景也没有漏掉
- 当前 answer 阶段已经不再是“链路跑通”，而是进入了“效果很强”的状态
...
| mixed v4 | - | 0.0045 | prompt 对齐 + ask_user 对照样本后 |
...
- 当数据和 prompt 真正对齐到“状态条件决策”这件事上时
- benchmark 指标会出现非常明显的同步提升
...
- `eval_loss` 很低说明训练是健康的
- 但真正判断 answer 是否够用，还是要看：
  - `answer_token_f1`
  - `answer_exact_match`
...
4. `handoff` 在 `mixed v4` 上已经非常强
5. `ask_user recall` 在 `mixed v4` 上出现了决定性改善
6. “同句不同上下文”的对照样本是这次提升的关键
7. `answer mixed v1` 也已经达到了很强的结果
8. benchmark 本身也需要持续校准，不能只看第一次输出

## 7. 现在最推荐的下一步

当前最推荐的顺序仍然是：

1. 把 `router mixed v4 + answer mixed v1` 作为当前最佳组合固定下来
2. 对 `answer` 也补一套 holdout benchmark，确认不是只在 mixed dev 上变强
3. 做端到端联调和真实查询 spot check

换句话说：

- 当前已经不需要继续盲目堆 router / answer 训练数据
- 更值得优先做的是：
  - 泛化验证
  - answer holdout benchmark
  - 主链路联调
